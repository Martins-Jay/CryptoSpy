...
class FontsMediumActions {
  ...
  generateFontOptions({ action, id, value, fontName, title }) {
    ...
    const html = `
      ...
      <button data-action="${action}"
        id="${id}"
        class="flex items-center justify-between px-6 py-2 rounded-lg border-0"
      >
        <!-- font title -->
        <div
          id="font-${value}-title"
          class="flex font-${fontName} text-sm tracking-wider md:cursor-pointer"
        >
          ${title}
        </div>
  ...
  renderHTML() {
    const { contentArea } = menuView.menuElements.medium;
    this.contentArea = contentArea;
    this.accordionContentEl = this.contentArea.querySelector(
      '#medium-menu-font-accordion-container'
    );

    if (!this.accordionContentEl.hasChildNodes()) {
      this.accordionContentEl.innerHTML = this.getHTML();
    }
  }

  toggleFontsAccordion() {
    const allAccordionEl = document.querySelectorAll('.accordion-content');
    this.fontAccordionContainerEl = this.accordionContentEl;
  ...
  updateActiveTheme(fontName) {
    ...
    this.activeThemeEl.textContent = fontName;
  }
...
export const fontsMediumActionsObj = new FontsMediumActions();
